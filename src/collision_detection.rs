...
use crate::{asteroids::Asteroid, health::Health, schedule::InGameSet, spaceship::Bullet};
...
impl Plugin for CollisionDetectionPlugin {
    fn build(&self, app: &mut App) {
        ...
        .add_systems(
            Update,
            (
                (handle_collisions::<Asteroid>, handle_collisions::<Bullet>),
...
fn handle_collisions<T: Component>(
    ...
    for (entity, collider) in query.iter() {
        for &collided_entity in collider.colliding_entities.iter() {
            ...
            // Entity collided with an entity of a different type.
...
fn apply_collision_damage(
    ...
    } in collision_event_reader.read()
    {
        ...
        let Ok(collision_damage) = collision_damage_query.get(collided_with) else {
            continue;
        };
